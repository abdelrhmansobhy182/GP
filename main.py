...
#######################################################
Selected = ['age','blood_pressure','blood_urea','serum_creatinine' ,'pus_cell_clumps' , 'bacteria' , 'hypertension', 'diabetes_mellitus' , 'coronary_artery_disease'
...
test_list = ['specific_gravity','albumin', 'sugar','red_blood_cells','pus_cell','blood_glucose_random','sodium','potassium' ,
...
for i in CategralTest:
    ...
    model = PredictionModel.trainModel(result, i, True)
    data = PredictionModel.predict(TestData, model, data, i)

for i in NumaricTest:
    result = PredictionModel.merge(train_data, data[i])
    TestData = PredictionModel.getTestData(result, i)
    model = PredictionModel.trainModel(result, i, False)
    data = PredictionModel.predict(TestData, model, data, i)
